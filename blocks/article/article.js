document.addEventListener("DOMContentLoaded", async () => {
  ...
  const description = new URLSearchParams(window.location.search).get(
    "description"
  );
  const isContentPage = path !== null; // Check if it's the content page

  // Elements for the article listing page
  if (!isContentPage) {
    const block = document.querySelector(".block");
    if (!block) return; // Exit if the block is not found
      ...
      articleListContainer.innerHTML = articlesToShow
        .map(
          ...
          <div class="article-item">
            ...
            <a href="/content/my-website/index/article-content?path=${encodeURIComponent(
              article.path
    ...
    function displayArticleContent() {
      ...
      if (descriptionContainer && description) {
        descriptionContainer.innerHTML = decodeURIComponent(description);
      }
    }
